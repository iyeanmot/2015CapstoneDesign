...
public class fire_hp2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (hp <= 0)
        {
            ...
            hp_check = false;
            fire_check = false;
            exting_man.GetComponent<getItem>().firecheck2 = false;
            // exting_man.GetComponent<getItem>().isSuccess = true;
            Debug.Log(exting_man.GetComponent<getItem>().firecheck3);
        }
        ...
        if (Time.time - Ontime < 0.5f)
        {
            ...
            gage.GetComponent<show_gage2>().fire_hp_delta = delta_hp;
        }
        //불 작아지기
        if (be_small)
        {
